	...
	void ShowFileDialog(bool* open, char* buffer, [[maybe_unused]] unsigned int buffer_size, FileDialogType type = FileDialogType::OpenFile) {
		...
		if (open) {
			...
			for (int i = 0; i < files.size(); ++i) {
                // Skip files that cause an exception when trying to access them
                try {
                    files[i].file_size();
                }
                catch (...) {
            ...
            std::vector<char> buf(selected_file_path.size() + 1); // +1 for null terminator
            std::strncpy(buf.data(), selected_file_path.c_str(), buf.size());
			ImGui::PushItemWidth(724);
			ImGui::InputText("##text", buf.data(), buf.size(), ImGuiInputTextFlags_ReadOnly);
